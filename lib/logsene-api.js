...
var logseneUri = process.env.LOGSENE_URI
var es
...
module.exports = {
  ...
  login: function _login (user, pass, cb) {
    ...
    var fn = function (err, res, body) {
      ...
      if (isDef(res) && res.statusCode === 200) { // are we good?
        ...
        out.trace('statusCode not 200' + res.body)
  ...
  getApps: function _getApps (apiKey, cb) {
    ...
    var fn = function (err, res, body) {
      ...
      if (isDef(res) && res.statusCode === 200) {
        // are we good?
...
>>>>>>> Encrypt ES _search requests (communicate with search receiver over https)
