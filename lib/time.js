...
var isDate = function _isDate (d) {
  if (d && moment.isMoment(d) && d.isValid()) {
    return true
  }
...
var isDuration = function _isDuration (duration) {
  if (/^((?![0-9]).)*$/.test(duration)) {
    return false // if it doesn't contain digits it cannot be duration
  }
...
var parseDuration = function _extractDuration (duration) {
  ...
  if (matched) {
    ...
    var ret = {}
    if (matched[1]) {
      ret.y = +matched[1].slice(0, -1)
    }
    if (matched[2]) {
      ret.M = +matched[2].slice(0, -1)
    }
    if (matched[3]) {
      ret.d = +matched[3].slice(0, -1)
    }
    if (matched[4]) {
      ret.h = +matched[4].slice(0, -1)
    }
    if (matched[5]) {
      ret.m = matched[5].indexOf('m') > -1 ? +matched[5].slice(0, -1) : +matched[5]
    }
    if (matched[6]) {
      ret.s = +matched[6].slice(0, -1)
    }
