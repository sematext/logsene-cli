      ...
      if (rangeContainsDuration(tr, sep)) {
        ...
        if (dur) {
          ...
          if (durPrefix === '+') {  // add duration to start
            ret.end = d1.clone().add(duration).toDate();  // Moment is mutable so we need to clone it
          } else if (durPrefix === '-') {  // subtract duration from start and set end to former start
            ret.end = ret.start;
 ...
 * @public
 */
var calculateRange = function _calculateRange(strTime, opts) {
  ...
  if (sepAt > -1) {  // it looks like range
    ...
    if (durationParsed) {
      ...
      out.trace('parse: found duration ' + stringify(durationParsed) + ' with ' + durPrefix + ' prefix');
